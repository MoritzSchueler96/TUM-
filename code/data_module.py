...
class GroupId2Name:
    ...
    def __init__(self, config):
        ...
        self.group_id2name = {x[1]: x[0] for x in df.drop_duplicates(subset=['Group Name', 'Facebook Id'])[['Group Name', 'Facebook Id']].values.tolist()}
...
class Collator:
    ...
    def collate(self, batch):
        ...
        encoded_texts = self.tokenizer([row['Text'] for row in features], padding=True, truncation=True, return_tensors="pt")
        encoded_classes = self.class_encoder.transform(list(get_classes_per_row(features, self.config)))
        
        return torch.FloatTensor(labels), encoded_texts, torch.FloatTensor(encoded_classes), features
...
def create_datasets(class_encoder, config={}):
    ...
    df['Text'] = df['Message'].fillna('').astype(str) + ' ' + df['Image Text'].fillna('').astype(str) + ' ' + \
                df['Link Text'].fillna('').astype(str) + ' ' + df['Description'].fillna('').astype(str)
    
    raw_records = df.to_dict('records')
    raw_labels = df[config['column_goldlabel']].to_list()
    
    # Split train / val_test date
    X_train, X_val_test, y_train, y_val_test = train_test_split(raw_records, raw_labels, test_size=0.4, random_state=42)
    ...
    return CrowdTangleDataset(X_train, y_train), \
...
class CrowdTangleDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        if self.trainset is None:
            ...
            self.trainset, self.valset, self.testset = create_datasets(self.class_encoder, self.config)
    ...
    def collate(self, batch):
        ...
        for row in features:
            encoded_texts.append(
                torch.tensor([self.vocab[token] for token in self.tokenizer(row['Text'])])
            )
        
        encoded_classes = self.class_encoder.transform(list(get_classes_per_row(features, self.config)))
        
        return torch.FloatTensor(labels),\
               pad_sequence(encoded_texts, batch_first=True, padding_value=self.vocab['<pad>']),\
...
class PlainCrowdTangleDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        if self.trainset is None:
            self.trainset, self.valset, self.testset = create_datasets(self.class_encoder, self.config)
            self._build_vocab()
            self.collator = PlainCollator(self.vocab, self.tokenizer, self.class_encoder, self.config)
